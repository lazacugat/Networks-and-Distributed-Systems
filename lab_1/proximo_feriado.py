...
def feriado_por_tipo(tipo):

    hoy = date.today()
    response = requests.get(get_url(hoy.year))
    data = response.json()
    
    feriados_tipo = next(
        (feriado for feriado in data if (feriado['mes'] == hoy.month and feriado['dia'] > hoy.day or feriado['mes'] > hoy.month) and (feriado['tipo'] == tipo))
    )
